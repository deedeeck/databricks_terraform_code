...
resource "aws_s3_bucket" "metastore" {
  bucket = "${local.prefix_uc}-metastore"
  acl    = "private"
  versioning {
    enabled = false
  }
  ...
  tags = merge(local.tags, {
    Name = "${local.prefix_uc}-metastore"
  })
}
...
data "aws_iam_policy_document" "passrole_for_uc" {
  ...
  statement {
    ...
    condition {
      ...
      values   = ["arn:aws:iam::${var.aws_account_id}:role/${local.prefix_uc}-uc-access"]
    }
  }
}
...
resource "aws_iam_policy" "unity_metastore" {
  policy = jsonencode({
    Version = "2012-10-17"
    Id      = "${local.prefix_uc}-databricks-unity-metastore"
    Statement = [
      {
        "Action" : [
          "s3:GetObject",
  ...
  tags = merge(local.tags, {
    Name = "${local.prefix_uc}-unity-catalog IAM policy"
  })
}
...
resource "aws_iam_policy" "sample_data" {
  policy = jsonencode({
    Version = "2012-10-17"
    Id      = "${local.prefix_uc}-databricks-sample-data"
    Statement = [
      {
        "Action" : [
          "s3:GetObject",
